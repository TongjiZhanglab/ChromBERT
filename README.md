...
Note: Currently, the `chrombert_prepare_env` command only prepares the recommended 1-kb resolution. For other resolutions, please download the dependencies (`{prefix}.hdf5.gz`, `{prefix}_pretrain.ckpt`, `{prefix}_region.bed`) manually and decompress using `gzip -d {prefix}.hdf5.gz`.
